...
import com.comet.keyboard.purchase.google.v2.Purchase;
...
//	public static DebugTracer mProfileTracer = new DebugTracer();
	...
	public void onCreate() {
		...
		try { crittercismConfig.put("shouldCollectLogcat", true); } catch (JSONException je) {}
		Crittercism.init(getApplicationContext(), getString(R.string.crittercism_app_id), crittercismConfig);
		Crittercism.setUsername(Utils.getDeviceID(this));
	}
...
//		BugSenseHandler.setup(this, "c4916d5e");
	 ...
	 * 
	 * @param activity
	 *            The activity that displayed the logo.
	 */
	...
	private void openVoiceInput() {
		...
		// Use Google Voice Typing with ICS or higher
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
			openVoiceTyping();
			return;
		}

		mVoiceIsShifted = getKeyboardView().getKeyboard().isShifted();
		mVoiceCapsLock = getKeyboardView().getCapsLock();

		if(mUseDragon) {
			// Use Dragon
			mKeyboardView.openDragonVoice();
		} else {
			// Use legacy Google voice API
			dismissAllPopupWindows(true);
			killActivities();

			if (!Utils.isExistVoiceRecognizeActivity(this)) {
				// Show toast message
				Toast.makeText(this, R.string.recongnizer_not_present,
						Toast.LENGTH_LONG).show();
				return;
			}
			
			// Set input language
			String lang = LanguageSelector.getLangNameFromCode(this, LanguageSelector.getLanguagePreference(this));
			if (!lang.equals(getResources().getString(R.string.lang_other))) {
				VoiceInput.setLanguage(lang);
			}
			VoiceInput.setRecognitionHandler(mVoiceHandler);
	...
	protected void commitText(CharSequence text) {
		...
		// Paste text to the TextEdit
		ExtractedText extractedText = inputConnection.getExtractedText(new ExtractedTextRequest(), 0);
		String sText;
		if (extractedText != null) {
			sText = extractedText.text.toString();
			inputConnection.setSelection(0, sText.length());
		}
...
//		mSuggestionsAlwaysShow = sharedPrefs.getBoolean("suggestions_always_show", true);
		...
		// Save Crittercism meta-data
		try {
			JSONObject metadata = new JSONObject();
			metadata.put("language", mLanguage);
			metadata.put("keyboard", mKeyboardLayoutId);
			metadata.put("theme", mTheme);
	...
	protected static boolean useUndoMenu() {
		if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH)
			return true;
	...
	public void onUpdateSelection(int oldSelStart, int oldSelEnd,
			int newSelStart, int newSelEnd, int composingStart, int composingEnd) {
	...
	/**
	 * Called after a successful purchase of translation credits. Updates the
	 * translator UI.
	 */
	public void billingCompleted() {
		int point;

		point = Purchase.getTransPoint(this) + TRANS_PURCHASE_INCREASE_UNIT;
		Assert.assertTrue(point >= 0);
		Purchase.putTransPoint(this, point);

		TranslatorView translatorView = getKeyboardView().mTranslatorView;
		if (translatorView != null)
			translatorView.onUpdateUI();
	}
	 ...
	 * 
	 * @param primaryCode
	 *            The primary keystroke code.
	 * @param keyCodes
	 *            All possible codes for this keystroke.
	 */
	@Override
...
//				if (mSmartSpaces && code == 109 && mComposing.toString().equalsIgnoreCase("co")) {
//					CharSequence dotcom = getTextBeforeCursor(inputConnection, 4);
//					if (dotcom.length() == 4
//							&& (dotcom.equals(". co") || dotcom.equals(". Co"))) {
//						inputConnection.commitText(mComposing, 1);
//						inputConnection.deleteSurroundingText(4, 0);
//						inputConnection.commitText(".", 1);
//						mComposing.setLength(0);
//						mComposing.append("co");
//					}
//				}
...
//		}
	...
	protected void onEnter() {
		commitComposing();
		// Google mail body field is "special".
		if (isGoogleMailBody() && Build.VERSION.SDK_INT < Build.VERSION_CODES.HONEYCOMB)
			onText("\r\n");
		else
	...
	protected void onWordSeparator(final int code) {
		...
		String committedWord = mCandidateView.getCommittedWord();
		if (committedWord != null) {
			committedWord += commit;
		} else
...
//		mProfileTracer.reset();
	 ...
	 * 
	 * @param orgWord
	 *            The word that replaced the undo word.
	 * @param undoWord
	...
	protected CharSequence pickDefaultCandidate() {
		...
		if (committedWord != null && mComposing != null && isDifferent) {
			if(useUndoMenu()) {
				...
				mCandidateView.updateUndoWord(committedWord, orgWord);
	 ...
	 * 
	 * @param index
	 *            The index of the suggestion to insert.
	 * @return The word inserted.
	 */
	 ...
	 *            The composition.
	 * @param newChar
	 *            The character following the composition.
...
//		Crittercism.leaveBreadcrumb("KeyboardService." + call);
